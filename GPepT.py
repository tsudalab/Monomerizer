...
# Argparse
parser = argparse.ArgumentParser(description='Train and generate sequences using a transformer model')
parser.add_argument('-canonical', action='store_true', help='Use only canonical sequences')
parser.add_argument('--input_data', type=str, help='Input data file', default='mols.txt')
...
finetune_sequences, all_sequences = [], []
for index, row in input_data.iterrows():
    if pd.notna(row["SEQUENCE"]):
        if args.canonical:
            if "X" in row["SEQUENCE"]:
                continue
        if row["ID"].startswith("Ecoli"):
            finetune_sequences.append(row["SEQUENCE"])
        all_sequences.append(row["SEQUENCE"])
finetune_sequences = list(set(finetune_sequences))
all_sequences = list(set(all_sequences))
print(f"Number of sequences in all: {len(all_sequences)}")
print(f"Number of sequences in finetune: {len(finetune_sequences)}")
...
class TokenEmbedding(torch.nn.Module):
    ...
    def __init__(self, d_model, number_of_tokens):
        ...
        self.embedding_layer = torch.nn.Embedding(
            num_embeddings=number_of_tokens, embedding_dim=d_model
        )
...
class MaskedMultiHeadedSelfAttention(torch.nn.Module):
    ...
    def __init__(self, embedding_dimension, number_of_heads):
        ...
        self.self_attentions = torch.nn.ModuleList(
            [
                MaskedSelfAttention(embedding_dimension, self.head_dimension)
        ...
        self.output_layer = torch.nn.Linear(
            number_of_heads * self.head_dimension, embedding_dimension
        )
    ...
    def forward(self, x, mask):
        ...
        self_attention_outputs = [
            self_attention(x, mask) for self_attention in self.self_attentions
        ]
...
class DecoderLayer(torch.nn.Module):
    ...
    def __init__(
        self, embedding_dimension, number_of_heads, feed_forward_dimension, dropout_rate
    ):
        super().__init__()
        self.embedding_dimension = embedding_dimension
        self.number_of_heads = number_of_heads
        ...
        self.multi_headed_self_attention = MaskedMultiHeadedSelfAttention(
...
class Tokenizer:
    ...
    def tokenize(self, sequence):
        pattern = r"[A-WYZ]|X\d+ter+|X\d+|<eos>"
        regex = re.compile(pattern)
        tokens = [token for token in regex.findall(sequence)]
        indices = []
        for token in tokens:
            self.__add_to_dict(token)
            indices.append(self.dictionary[token])
        return indices
...
class Trainer:
    ...
    def train(self, data: List[str], train_level, epochs, batch_size, patience):
        # Create lists to store training losses, validation losses, and test losses
        train_losses = []
        val_losses = []
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(train_data), batch_size):
                sequence_tensor = torch.tensor(
                    train_data[i : i + batch_size], dtype=torch.long
                )
                ...
                mask_tensor[
                    sequence_tensor == self.tokenizer.character_to_token("<pad>")
                ] = 0
            ...
            for batch in batches:
                ...
                input_tensor = torch.zeros(
                    (batch_size, self.model.max_sequence_length + 1), dtype=torch.long
                )
                ...
                model_output, target = self.model.forward(
                    x=input_tensor.to(get_device()), mask=mask_tensor.to(get_device())
                )
            ...
            # Print epoch info including time, training loss, validation loss, and test loss
            print(
                f"Epoch: {epoch}, Time: {end_time - start_time:.2f}s, Training Loss: {epoch_loss:.4f}, Validation Loss: {val_loss:.4f}, Perplexity: {perplexity:.4f}"
            )
            ...
            if val_loss < best_val_loss:
                ...
                consecutive_no_improvement = 0
                self.model.save_checkpoint(model_name)
            else: # do not save model
    ...
    def evaluate(self, data: List[str], batch_size):
        ...
        for i in range(0, len(data), batch_size):
            ...
            eval_batches.append((sequence_tensor, mask_tensor))
        ...
        # Compute the average loss over all evaluation batches
...
class Generator:
    ...
    def generate(
        ...
        return "".join(
            [self.tokenizer.token_to_character(token) for token in generated_tokens]
        )
...
class Runner(torch.nn.Module):
    ...
    def run(self, train_level, data):
        # Create the tokenizer
        tokenizer = Tokenizer(all_sequences)
        ...
        elif train_level == "finetune":
            model = AutoregressiveWrapper.load_checkpoint(model_name).to(get_device())
            freeze_layers_except_last(model)
        
        # Create the training data
        input_string = "<eos>".join(data)

        tokenized_and_padded_training_data = tokenize_and_pad_training_data(
            max_sequence_length, tokenizer, input_string
        )
        sequences = create_training_sequences(
            max_sequence_length, tokenized_and_padded_training_data
        )

        # Train the model
        optimizer = torch.optim.Adam(model.parameters(), lr=0.00005)
        trainer = Trainer(model, tokenizer, optimizer)
        print(f"{train_level} starts")

        loss_per_epoch, val_loss_per_epoch = trainer.train(
            sequences, train_level, epochs=30, batch_size=16, patience=2,
        )

        # Plot the loss per epoch in log scale
        plt.plot(loss_per_epoch, label="Training Loss", color="blue")
        plt.plot(val_loss_per_epoch, label="Validation Loss", color="red")
        plt.title(train_level)
        plt.yscale("log")
        plt.ylabel("Loss")
        plt.xlabel("Epoch")
        plt.legend()
        plt.savefig(f"Lossplot_{train_level}.jpg")
        plt.close()

        if train_level == "finetune":
            generator = Generator(model, tokenizer)
            for i in range(1,16):
                temperature = (i)/10
                print(f"Generating sequences with temperature {temperature}")
                for _ in range(10000):
                    ...
                    example_sequence = generated_text.replace('<pad>', '').split('<eos>')[1]
                    writer.writerow([temperature, example_sequence])
...
def freeze_layers_except_last(model):
    for name, param in model.named_parameters():
        if "lm_head" not in name:  # Exclude the last layer (lm_head)
            param.requires_grad = False

# Set output_file to datetime
output_file = f"generated_{time.strftime('%Y%m%d-%H%M%S')}.tsv"
with open(output_file, 'w', newline='', encoding='utf-8') as tsvfile:
    writer = csv.writer(tsvfile, delimiter='\t')
    writer.writerow(["ID", "SEQUENCE"])
    Runner().run("pretrain", all_sequences)
    Runner().run("finetune", finetune_sequences)
